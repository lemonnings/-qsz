...
var boss_speed : float = SettingMoster.slime("speed") * 1 # Boss移动速度，可以调整
...
# 子弹场景，需要预加载
const STRAIGHT_BULLET = preload("res://Scenes/bullet.tscn") # 直线子弹
...
const NORMAL_BARRAGE_BULLET_SCENE = preload("res://Scenes/bullet.tscn") # 普通弹幕子弹
...
func _attack_charge():
	...
	var charge_speed = boss_speed * 12 # 冲锋速度
...
# 检查怪物是否在可伤害范围内（超出视野30px就不判定了），对于大型单位提高一点判定范围
