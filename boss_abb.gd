...
# var hpMax : float = SettingMoster.bat("hp") * 1000 # Boss最大生命值，可以调整
...
# 		progress_bar.position = global_position + Vector2(0, -50) #血条位置调整
# 		health_bar_shown = true
# 		progress_bar.top_level = true
# 	elif progress_bar and progress_bar.is_inside_tree():
# 		progress_bar.position = global_position + Vector2(0, -50) 
...
# 	if [3,4,7,9].has(attack_type): # 7是旋转激光，9是黑洞
...
# 		attack_indicator = Node2D.new() # 创建一个新的Node2D作为指示器的容器
# 		# 创建外层红色线条
# 		var outer_line = Line2D.new()
# 		outer_line.name = "OuterLine"
# 		outer_line.default_color = Color(1.0, 0.1, 0.0, 0.25) # 红色
# 		outer_line.width = 23
# 		# 创建内层橙黄色线条
# 		var inner_line = Line2D.new()
# 		inner_line.name = "InnerLine"
# 		inner_line.default_color = Color(1.0, 1, 0, 0.25) # 橙黄色
# 		inner_line.width = 16
# 		# 设置初始点
# 		outer_line.set_points([Vector2(0, 0), Vector2(100, 0)])
# 		inner_line.set_points([Vector2(0, 0), Vector2(100, 0)])
...
# 	outer_line_node = attack_indicator.get_node_or_null("OuterLine") as Line2D
# 	inner_line_node = attack_indicator.get_node_or_null("InnerLine") as Line2D
# 	if outer_line_node and inner_line_node:
# 		outer_line_node.clear_points()
# 		inner_line_node.clear_points()
# 		var player_pos = PC.player_instance.global_position
...
# 					outer_line_node.add_point(direction * 600)
# 					inner_line_node.add_point(Vector2.ZERO)
# 					inner_line_node.add_point(direction * 600)
# 			4: # 冲锋
# 				outer_line_node.add_point(Vector2.ZERO)
# 				outer_line_node.add_point(dir_to_player * 400)
# 				inner_line_node.add_point(Vector2.ZERO)
# 				inner_line_node.add_point(dir_to_player * 400)
# 			7: # 旋转激光
# 				outer_line_node.add_point(Vector2.ZERO)
...
# 		# await get_tree().create_timer(1.0).timeout # 预留1秒显示攻击范围
...
# 	# 创建双层线条效果
# 	var outer_line_node = Line2D.new()
# 	var inner_line_node = Line2D.new()
	
# 	attack_indicator.add_child(outer_line_node)
# 	attack_indicator.add_child(inner_line_node)
	
# 	# 设置外层红色线条
# 	outer_line_node.default_color = Color(1.0, 0.1, 0.0, 0.35)
# 	outer_line_node.width = 40.0
	
# 	# 设置内层橙黄色线条
# 	inner_line_node.default_color = Color(1.0, 1, 0, 0.35) # 橙黄色
# 	inner_line_node.width = 34.0
	
# 	# 绘制直线
# 	var attack_range_length = 1000.0
# 	var line_end = global_position + direction * attack_range_length
	
# 	outer_line_node.add_point(Vector2.ZERO)
# 	outer_line_node.add_point(to_local(line_end))
# 	inner_line_node.add_point(Vector2.ZERO)
# 	inner_line_node.add_point(to_local(line_end))
	
# 	# 初始状态：完全透明
# 	outer_line_node.modulate.a = 0.0
# 	inner_line_node.modulate.a = 0.0
# 	attack_indicator.visible = true
	
# 	# 从里到外逐渐生成的动画
# 	var appear_tween = create_tween()
# 	appear_tween.set_parallel(true) # 允许并行动画
	
# 	# 内层线条先出现（0.3秒）
# 	appear_tween.tween_property(inner_line_node, "modulate:a", 1.0, 0.2)
# 	# 外层线条稍后出现（0.2秒延迟后0.3秒）
# 	appear_tween.tween_property(outer_line_node, "modulate:a", 1.0, 0.15).set_delay(0.12)
...
# 	disappear_tween.set_parallel(true) # 允许并行动画
...
# 		if child is Line2D:
# 			if child.width >= 34.5: # 外层线条
# 				outer_lines.append(child)
# 			elif child.width >= 28: # 内层线条
# 				inner_lines.append(child)
	
# 	# 外层线条先消失（0.2秒）
# 	for outer_line in outer_lines:
# 		if is_instance_valid(outer_line):
# 			disappear_tween.tween_property(outer_line, "modulate:a", 0.0, 0.08).set_delay(0.05)
	
# 	# 内层线条稍后消失（0.1秒延迟后0.2秒）
# 	for inner_line in inner_lines:
# 		if is_instance_valid(inner_line):
# 			disappear_tween.tween_property(inner_line, "modulate:a", 0.0, 0.08).set_delay(0.05)
...
# 		var player_pos = PC.player_instance.global_position
# 		var boss_pos = global_position
# 		var attack_range_length = 800.0
# 		var line_width_tolerance = 28
...
# 		var direction = base_direction.rotated(deg_to_rad(i * 20))
...
# 		# 设置外层红色线条
# 		outer_line_node.default_color = Color(1.0, 0.1, 0.0, 0.35)
# 		outer_line_node.width = 34.5
		
# 		# 设置内层橙黄色线条
# 		inner_line_node.default_color = Color(1.0, 1, 0, 0.35) # 橙黄色
# 		inner_line_node.width = 28
...
# 	for child in attack_indicator.get_children():
# 		if child is Line2D:
# 			var delay = (line_index % 2) * 0.1 # 内外层错开
# 			if child.width >= 28 and child.width < 34.5: # 内层线条先出现
# 				appear_tween.tween_property(child, "modulate:a", 1.0, 0.15).set_delay(delay)
# 			else: # 外层线条稍后出现
# 				appear_tween.tween_property(child, "modulate:a", 1.0, 0.15).set_delay(delay + 0.1)
# 			line_index += 1
...
# 		var line_width_tolerance = 25.0 # 线条宽度容差，可以调整
...
# 		bullet.target_direction = global_position.direction_to(PC.player_instance.global_position) # 持续瞄准
...
# 	var base_dir = global_position.direction_to(PC.player_instance.global_position)
...
# 		get_parent().add_child(bullet)
# 		bullet.global_position = global_position
# 		bullet.target_direction = global_position.direction_to(PC.player_instance.global_position)
...
# 			laser_h.default_color = Color(1.0, 1, 0, 0.35)
# 			laser_h.width = 5
...
# 			# Example: laser_h.setup_beam_segment(global_position + Vector2(-screen_w/2, y_pos), global_position + Vector2(screen_w/2, y_pos), Color(1.0, 1, 0, 0.35), 5)
...
# 			laser_v.default_color = Color(1.0, 1, 0, 0.35)
# 			laser_v.width = 5
...
# 	# 扩展边界20px，只有超出这个范围的怪物才不能被伤害
# 	var damage_margin = 20.0
# 	left_bound -= damage_margin
# 	right_bound += damage_margin
# 	top_bound -= damage_margin
# 	bottom_bound += damage_margin
