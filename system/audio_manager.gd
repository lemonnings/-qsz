...
@export var bg_volume: float = 1.0 # 环境音量
...
func setup_audio_buses() -> void:
	...
	# 确保BG总线存在（环境音量）
	if AudioServer.get_bus_index(BG_BUS) == -1:
		AudioServer.add_bus()
		var bg_index = AudioServer.get_bus_count() - 1
		AudioServer.set_bus_name(bg_index, BG_BUS)
		AudioServer.set_bus_send(bg_index, MASTER_BUS)
...
func set_bg_volume(volume: float) -> void:
	bg_volume = clamp(volume, 0.0, 1.0)
	apply_bg_volume()
	volume_changed.emit(BG_BUS, bg_volume)
	save_audio_settings()
...
func apply_bg_volume() -> void:
	var bg_bus_index = AudioServer.get_bus_index(BG_BUS)
	if bg_bus_index == -1:
		push_error("BG音频总线不存在")
		return
	
	if bg_volume <= 0.0:
		AudioServer.set_bus_mute(bg_bus_index, true)
	else:
		AudioServer.set_bus_mute(bg_bus_index, false)
		var db = linear_to_db(bg_volume)
		AudioServer.set_bus_volume_db(bg_bus_index, db)
...
func toggle_bg_mute() -> void:
	var bg_bus_index = AudioServer.get_bus_index(BG_BUS)
	if bg_bus_index == -1:
		push_error("BG音频总线不存在")
		return
	
	var is_muted = AudioServer.is_bus_mute(bg_bus_index)
	AudioServer.set_bus_mute(bg_bus_index, not is_muted)
	# 发送信号通知UI更新静音状态
	volume_changed.emit(BG_BUS, bg_volume if not is_muted else 0.0)
