...
var base_speed: float = SettingMoster.lantern("speed")
var speed: float
var hpMax: float = SettingMoster.lantern("hp")
var hp: float = SettingMoster.lantern("hp")
var atk: float = SettingMoster.lantern("atk")
var get_point: int = SettingMoster.lantern("point")
var get_exp: int = SettingMoster.lantern("exp")
var get_mechanism: int = SettingMoster.lantern("mechanism")
...
func _physics_process(delta: float) -> void:
	if hp <= 0:
		...
		if not is_dead: # Add this check
			...
			if SettingMoster.lantern("itemdrop") != null:
				for key in SettingMoster.lantern("itemdrop"):
					var drop_chance = SettingMoster.lantern("itemdrop")[key] * drop_rate_multiplier
					if randf() <= drop_chance:
						Global.emit_signal("drop_out_item", key, 1, global_position)
				# for item in SettingMoster.lantern("itemdrop"):
				# 	if randf() <= SettingMoster.lantern("itemdrop")[item]:
