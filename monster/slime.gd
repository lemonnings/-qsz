...
func _ready():
	...
	var shadow = CharacterEffects.create_shadow(self , 22.0, 6.0, 10.0)
		...
		if not is_dead: # Add this check
			...
			if SettingMoster.slime_green("itemdrop") != null:
				for key in SettingMoster.slime_green("itemdrop"):
					var drop_chance = SettingMoster.slime_green("itemdrop")[key] * SettingMoster.get_item_drop_rate_multiplier(key, drop_rate_multiplier)
