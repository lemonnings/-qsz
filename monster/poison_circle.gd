...
var instant_damage_on_enter: bool = true # 进入时是否立刻判定一次伤害
...
func _process(delta: float) -> void:
	...
	# 玩家站在毒圈内时获得解毒buff → 立刻净化
	if player_inside and BuffManager.has_buff("boss_a_detox"):
		_purify()
		return
	...
	if player_inside:
		...
		if tick_timer >= tick_interval:
			...
			PC.player_hit_ignore_invincible(dmg, final_attacker, source_name)
...
func _on_body_entered(body: Node2D) -> void:
	if body is CharacterBody2D:
		# 拥有解毒buff时踩上毒圈 → 净化销毁毒圈，不造成伤害
		if BuffManager.has_buff("boss_a_detox"):
			_purify()
			return
		player_inside = true
		tick_timer = 0.0
		# 入场是否立刻判定一次伤害（不触发无敌）
		if instant_damage_on_enter:
			var dmg = max(1, int(damage_per_tick))
			var final_attacker: Node2D = attacker if is_instance_valid(attacker) else self
			PC.player_hit_ignore_invincible(dmg, final_attacker, source_name)
