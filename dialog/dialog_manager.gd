...
var _left_click_block_until_msec: int = 0 # 内联对话开场保护，防止左键跳过渐入和第一句
...
func _read_dialog_data_from_csv(file_path: String) -> Array:
	...
	if not FileAccess.file_exists(file_path):
		printerr("Dialog file does not exist: ", file_path)
		return []

	var file = FileAccess.open(file_path, FileAccess.READ)
	if file == null:
		printerr("Error opening CSV dialog file: ", file_path, " open_error=", FileAccess.get_open_error())
		return []
...
func _display_text_typewriter(text_to_display: String):
	...
	dialog_text_label.text = text_to_display
	dialog_text_label.visible_characters = 0
	
	# 使用 get_total_character_count() 获取实际可见字符数（排除 BBCode 标签）
	var total_chars = dialog_text_label.get_total_character_count()
	var duration = total_chars * text_display_speed
	
	current_text_tween = get_tree().create_tween()
	current_text_tween.tween_property(dialog_text_label, "visible_characters", total_chars, duration)
...
func _input(event: InputEvent): # 确保为此节点设置 process_input(true) 或全局处理输入
	...
	if is_left_click and _is_story_skip_mouse_event(event):
		return
	if is_left_click and Time.get_ticks_msec() < _left_click_block_until_msec:
		return
...
func start_dialog_from_data(data: Array):
	...
	is_animating_illustrations = false
	choices_already_shown = false
	_advance_cooldown_end = 0
	_left_click_block_until_msec = Time.get_ticks_msec() + 750
