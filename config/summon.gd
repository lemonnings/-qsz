...
func create_sword_spirit_bullet(direction: Vector2) -> void:
	...
	bullet.rotation = direction.angle()
	# 修习树领悟篇：唤灵系伤害提升加成
	var final_damage = PC.pc_atk * damage_multiplier * (1.0 + PC.summon_damage_multiplier + Global.study_summon_damage_bonus + Global.get_achievement_summon_damage_bonus())
	bullet.summon_damage = final_damage
	var bullet_size = Global.get_attack_range_multiplier() * PC.summon_bullet_size_multiplier
	bullet.set_bullet_scale(Vector2(bullet_size, bullet_size))
	if bullet.has_method("set_speed"):
