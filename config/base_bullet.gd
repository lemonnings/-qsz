...
#把现有的所有武器的升级方式修改为：每级进度9000+当前武器lv，初始每秒提升400。
#每级提升3%武器伤害，选择了升级选项中的武器升级，相当于直接+下一级的完整经验
#此外，绿色额外提升1%武器伤害，蓝2%武器伤害，紫3%武器伤害+每秒提升40获取进度，金4%武器伤害+每秒提升80获取进度，红5%武器伤害+每秒提升160获取进度
#每升级到4的倍数，可以进行一次进阶升级（现有是5的倍数可进阶1次）
