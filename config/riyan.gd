...
func _ready() -> void:
	...
	if player_node:
		...
		Global.connect("riyan_damage_triggered", Callable(self, "_on_damage_timer_timeout"))
		...
		draw_node.draw.connect(func():
			var color_fill = Color(1.0, 0.6, 0.0, 0.3) # 浅橙色填充
			var color_border = Color(1.0, 0.6, 0.0, 0.8) # 橙色描边
...
func _on_damage_timer_timeout() -> void:
	if player_node:
		var damage_amount: float = PC.pc_atk * 0.2 + PC.riyan_hp_max_damage
